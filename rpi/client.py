...
from common.exceptions import BadDataError, YozakuraTimeoutError, NoMbedError,\
    NoConnectionError, MotorCountError, NoDriversError, DynamixelError
...
class Client(object):
    ...
    def _get_arm_data(self, ignore=False):
        ...
        if self.arm is not None:
            positions, servo_vii = self.arm.get_data()
        else:
            self._logger.debug("Arm is not connected")
            positions = servo_vii = [None] * 3

        if ignore:
            float_arm_data = [None] * 33
        else:
        ...
        thermo_l = float_arm_data[0:16]
        thermo_r = float_arm_data[16:32]
