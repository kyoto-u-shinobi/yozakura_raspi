...
# Used in rpi.devices.ADConverter
class ADCConfigurationBits(ctypes.Structure):
    """
    The bits for the configuration register of the MCP3425 ``ADConverter``.
    ...
    """
    _fields_ = [("ready", ctypes.c_uint8, 1),    # Conversion ready flag
                ("channel", ctypes.c_uint8, 2),  # Channel selection; unused
                ("cont", ctypes.c_uint8, 1),     # Conversion mode bit
                ("rate", ctypes.c_uint8, 2),     # Sample rate selection bits
                ("gain", ctypes.c_uint8, 2)]     # PGA gain selector bits


class ADCConfiguration(ctypes.Union):
    """
    The union for the configuration register of ``ADConverter``.

    The ADC used here is the Microchip MCP3425 16-bit Analog-to-Digital
    Converter. The configuration register details are shown on page 14 of the
    datasheet. [1]_

    The MCP3425 has an 8-bit wide configuration register to select for: PGA
    gain, conversion rate, and conversion mode. This register allows the user
    to change the operating condition of the device and check the status of
    the device operation. The user can rewrite the configuration byte any time
    during the device operation.

    Bit Fields
    ----------
    ready : bool
        Conversion ready flag. If in oneshot mode, it needs to be set to one
        before reading. If in continuous conversion mode, it is set to one
        after a conversion has been read by the Master, and zero if a new
    ...
    """
    _fields_ = [("b", ADCConfigurationBits),
                ("as_byte", ctypes.c_uint8)]
